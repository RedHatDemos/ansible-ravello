...
def create_blueprint_from_template(client, module):
    ...
    rand_str = lambda n: ''.join([random.choice(string.lower()) for i in range(n)])
    ...
    if check_item_exists(read_app, 'network.subnets'):
        netlist = []
...
def make_checker(valid):
    checker_fn = None
    if not callable(valid):
        if not type(valid) is list:
            valid = [valid]
        checker_fn = lambda val: val in valid
    else:
        checker_fn = valid
    return checker_fn

def assert_item_exists(json_item, jspath, **kwargs):
    fail_msg = from_kwargs(kwargs, 'fail_msg', 
        "missing or invalid parameter: " + jspath)
    if not check_item_exists(json_item, jspath, **kwargs):
        raise Exception(fail_msg)
    return

def check_item_exists(json_item, jspath, **kwargs):
    path_exists = json_path_contains(json_item, jspath)
    valid = from_kwargs(kwargs, 'valid', lambda n: True)
    is_valid = make_checker(valid)
    result = path_exists and \
       is_valid(ravello_template_get(json_item, jspath))
    return result

def set_default_if_missing(json_item, jspath, default, **kwargs):
    path_exists = json_path_contains(json_item, jspath)
    valid = from_kwargs(kwargs, 'valid', lambda n: True)
    is_valid = make_checker(valid)
    if path_exists:
        item = ravello_template_get(json_item, jspath)
        if not is_valid(item):
            raise Exception("invalid parameter: " + \
                jspath + ": " +  json.dumps(item))
    else: 
        ravello_template_set(json_item, jspath, default)
    return 

def assert_hd_valid(client, module, hd):
    assert_item_exists(hd, 'index')
    assert_item_exists(hd, 'size.value', 
            valid=(lambda n: ((type(n) is int) and (n > 0))))
    set_default_if_missing(hd, 'type', 'DISK', valid=['DISK','CDROM'])
    set_default_if_missing(hd, 'controller', 'virtio', valid=['virtio', 'ide'])
    set_default_if_missing(hd, 'boot', False ,valid=[True, False])
    set_default_if_missing(hd, 'name', ('Disk' + hd['name']))
    set_default_if_missing(hd, 'size.unit', 'GB',valid=['GB', 'MB'])
...
def assert_nic_valid(client, module, nic):
    assert_item_exists(nic, 'ipConfig')
    assert_item_exists(nic, 'device.index')
    set_default_if_missing(nic, 'device.name', 
              'eth' + str(nic['device']['index']))
    set_default_if_missing(nic, 'device.deviceType', 'virtio',
              valid=['virtio', 'e1000'])
    set_default_if_missing(nic, 'device.useAutomaticMac', True,
               valid=[True, False])
    if not (nic['device']['useAutomaticMac'] or \
            check_item_exists(nic, 'device.mac')):
        module_fail('ERROR useAutomaticMac set to False but ' + \
                    'no static mac set for VM %s NIC index')
    auto_ip = check_item_exists(nic, 'ipConfig.autoIpConfig')
    static_ip = check_item_exists(nic, 'ipConfig.staticIpConfig')
    if static_ip:
        assert_item_exists(nic, 'ipConfig.staticIpConfig.ip')
        assert_item_exists(nic, 'ipConfig.staticIpConfig.mask')    
    if (auto_ip == static_ip):
        module_fail('Error: exactly one of [autoIpConfig,staticIpConfig] required')

def assert_vm_valid(client, module, vm):
    # Check Template Valid
    assert_item_exists(vm, 'numCpus')
    assert_item_exists(vm, 'memorySize.value')
    assert_item_exists(vm, 'supportsCloudInit', 
            fail_msg='Error: Template must support cloudInit')
    assert_item_exists(vm, 'hardDrives')
    assert_item_exists(vm, 'networkConnections')
    set_default_if_missing(vm, 'description', '')
    set_default_if_missing(vm, 'memorySize.unit', 'GB')
    set_default_if_missing(vm, 'stopTimeout', 300)
    set_default_if_missing(vm, 'bootOrder', ['DISK', 'CDROM'])
    # Set vm tag if def'd
    if 'tag' in vm:
    ...
    if 'suppliedServices' in vm:
      for svc in vm['suppliedServices']:
        assert_item_exists(svc, 'name')
        assert_item_exists(svc, 'portRange')
    # add vm to app
    return vm
...
def update_app_with_internal_luids(client, module, app_request, appID):
    ...
    for vm in app_request['design']['vms']:
        ...
        for nic in vm['networkConnections']:
            if check_item_exists(nic, 'ipConfig.autoIpConfig.reservedIp'):
