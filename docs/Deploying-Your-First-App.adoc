...
`ansible-ravello/workdir/my.first.app/`.  Within that work directory will be an 
...
```
./inventory/my-first-app --list | gawk 'match($0, /proxyFqdn/){print substr($2,2,length($2)-3)}' | uniq
```
...
```
user@localhost $ ssh -i workdir/my.first.app/my.first.app.id_rsa cloud-user@<bastion-public-fqdn>
